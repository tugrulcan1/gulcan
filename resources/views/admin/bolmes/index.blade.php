        ...
        <div class="row">
            <div class="col-lg-12">
                <div id="projectSummary"
                    data-list='{"valueNames":["projectName","assigness","start","deadline","task","projectprogress","status","action"],"bolme":12,"pagination":true}'>
                    <div class="row justify-content-between mb-4 gx-6 gy-3 align-items-center">
                        <div class="col-auto">
                            <h2 class="mb-0">Sayfalar<span class="fw-normal text-700 ms-3">({{ count($bolmes) }})</span>
                            </h2>
                        </div>
                        <div class="col-auto">
                            <div class="col-auto">
                                <a class="btn btn-primary px-5" href="{{ route('admin.bolmes.create') }}">
                    ...
                    <div class="card shadow-none border border-300 my-4 p-5">
                                ...
                                <tbody class="list" id="project-list-table-body">
                                    @foreach ($bolmes as $key => $bolme)
                                        <tr class="position-static">
                                            ...
                                            <td>
                                                @if (in_array('GetBolmeById', $userPermissions) && in_array('UpdateBolme', $userPermissions))
                                                    <a href="{{ route('admin.bolmes.edit', $bolme->id) }}"
                                                        class="btn btn-sm btn-primary">Güncelle</a>
                                                @elseif (in_array('GetBolmeById', $userPermissions))
                                                    <a href="{{ route('admin.bolmes.edit', $bolme->id) }}"
                                                        class="btn btn-sm btn-primary">Önizle</a>
                                                @endif
                                                @if (in_array('DeleteBolme', $userPermissions))
                                                    <!-- Silme işlemi için modal -->
                                                    <button type="button" class="btn btn-sm btn-danger"
                                                        data-bs-toggle="modal"
                                                        data-bs-target="#deleteModal{{ $bolme->id }}">
                                                        Sil
                                                    </button>
                                                @endif
                                                ...
                                                <!-- Silme işlemi için modal -->
                                                <div class="modal fade" id="deleteModal{{ $bolme->id }}" tabindex="-1"
                                                    aria-labelledby="deleteModalLabel{{ $bolme->id }}"
                                                    aria-hidden="true">
                                                    <div class="modal-dialog">
                                                        <div class="modal-content">
                                                            <div class="modal-header">
                                                                <h5 class="modal-title"
                                                                    id="deleteModalLabel{{ $bolme->id }}">Sil
                                                                </h5>
                                                                <button type="button" class="btn p-1"
                                                                    data-bs-dismiss="modal" aria-label="Close">
                                                                    <svg class="svg-inline--fa fa-xmark fs--1"
                                                                        aria-hidden="true" focusable="false"
                                                                        data-prefix="fas" data-icon="xmark" bolme="img"
                                                            ...
                                                            <div class="modal-footer">
                                                                <form
                                                                    action="{{ route('admin.bolmes.destroy', $bolme->id) }}"
                        ...
                        <div
                            ...
                            <div class="d-flex"><button class="bolme-link" data-list-pagination="prev"><span
                                        class="fas fa-chevron-left"></span></button>
                                <ul class="mb-0 pagination"></ul><button class="bolme-link pe-0"
